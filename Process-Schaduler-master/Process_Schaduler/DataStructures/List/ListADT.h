...
class LinkedList : public ListADT<type> {
private:
	int count;
	Node<type>* head;
public:
	...
	bool pop(T& item) {     // complexity O(n)
		if (!head) {
			return false;
		}
	...
	bool remove_entry(T item) {      // Complexity O(n) in the worst case. The complexity is a funtion of pos
	...
	bool replace(int pos, T new_item) {  // Complexity O(n) in the owrst case. The complexity is a funtion of pos
		Node<type>* curr_node = head;
		if ((pos < 0) || (pos >= count)) {
			return false;
		}
